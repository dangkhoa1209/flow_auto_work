...
- WorkBench dual corpus: root `graphify-out/` is this app; customer checkouts use **sibling** `project/<user>/<slug>/graphify-out/` (never inside `source/`). Rebuild: `scripts/graphify-work-project.sh <user>/<slug>`. Auto after clone/BA pull (**background** — BA/Work do not wait for update to finish). **BA Chat, workflow, Create Data plan, and Work (/dev)** share one rule: `code_map_query` is required when the file is unknown; the agent chooses the locator. The API does not pre-query or inject a raw graph dump. **Create issue** is chat-only (no source / graphify). Details in `.cursor/rules/graphify.mdc`.
