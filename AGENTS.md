...
- WorkBench dual corpus: root `graphify-out/` is this app; customer checkouts use **sibling** `project/<user>/<slug>/graphify-out/` (never inside `source/`). Rebuild: `scripts/graphify-work-project.sh <user>/<slug>`. Auto after clone/BA pull. **BA Chat / workflow / Create issue** inject a graphify query map into the agent prompt before Cursor runs. Details in `.cursor/rules/graphify.mdc`.
